...
final class MMDB_Tests: XCTestCase {
    func readAllAcceptErrors( forResource: String, withExtension: String = "mmdb", subdirectory: String) throws {
        guard let fileURL = Bundle.module.url(forResource: forResource, withExtension: withExtension, subdirectory: subdirectory),
            ...
            XCTFail("Failed to open MMDB \(Bundle.module.url(forResource: forResource, withExtension: withExtension, subdirectory: subdirectory))")
            return
        }
        ...
        try mmdb.enumerate{ bits, count in
            ...
            switch try mmdb.search(value: bits, bits: count) {
            case .value(let v):
                if case let .map(m) = v,
...
//                v.dump()
    ...
    func readAll( forResource: String, withExtension: String = "mmdb", subdirectory: String) throws {
        ...
        try mmdb.enumerate{ (bits:[UInt32], count: Int) in
            ...
            guard case let .value(v) = try mmdb.search(value: bits, bits: count) else {
    ...
    func testGeoIP2CountryTest() throws {
        ...
        guard case let .value(norsk) = try mmdb.search(value: 0x2a02f30000000000, bits: 64) else {
        ...
        guard case .notFound = try mmdb.search( starting: mmdb.ipv4Root, value: 0xc0000200 << 32, bits: 32) else {
...
//    }
    ...
    func testCyclicDataStructure() throws {
        try readAllAcceptErrors(forResource: "cyclic-data-structure", subdirectory: "bad-data/maxminddb-golang")
    }
    
    func testInvalidBytesLength() throws {
        try readAllAcceptErrors(forResource: "invalid-bytes-length", subdirectory: "bad-data/maxminddb-golang")
    }

    func testInvalidDataRecordOffset() throws {
        try readAllAcceptErrors(forResource: "invalid-data-record-offset", subdirectory: "bad-data/maxminddb-golang")
    }

    func testInvalidMapKeyLength() throws {
        try readAllAcceptErrors(forResource: "invalid-map-key-length", subdirectory: "bad-data/maxminddb-golang")
    }

    func testInvalidStringLength() throws {
        try readAllAcceptErrors(forResource: "invalid-string-length", subdirectory: "bad-data/maxminddb-golang")
    }

    func testMetadataIsAnUInt128() throws {
        try readAllAcceptErrors(forResource: "metadata-is-an-uint128", subdirectory: "bad-data/maxminddb-golang")
    }

//    func testUnexpectedBytes() throws {
//        try readAllAcceptErrors(forResource: "unexpected-bytes", subdirectory: "bad-data/maxminddb-golang")
//    }
...
//    }
    ...
    func testOffsetIntegerOverflow() throws {
        try readAllAcceptErrors(forResource: "libmaxminddb-offset-integer-overflow", subdirectory: "bad-data/maxminddb-golang")
    }

    func testSomeCityDb() throws {
        let url = Bundle.module.url(forResource: "GeoLite2-City", withExtension: "mmdb", subdirectory: "my-test")!
        print(url)
        let db = try MMDB(from: url)
        print(try db.search(address: "79.233.163.5"))
//        print(db?.search(address: "47.56.22.133"))
//        readAllAcceptErrors(forResource: "GeoLite2-City", subdirectory: "my-test")
    }
